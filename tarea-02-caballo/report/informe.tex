...
confirmando computacionalmente la identidad de
Chapman--Kolmogorov\footnote{Para cualquier par de tiempos
$m,n\geq0$, la ecuación de Chapman--Kolmogorov establece que
$P^{(m+n)}_{xy}=\sum_{z\in S}P^{(m)}_{xz}\,P^{(n)}_{zy}$: la
probabilidad de pasar de $x$ a $y$ en $m+n$ pasos es la suma, sobre
todos los estados intermedios $z$, del producto de la probabilidad de
pasar de $x$ a $z$ en $m$ pasos por la de pasar de $z$ a $y$ en $n$
pasos. Para $m=n=1$ se reduce a
$(P^2)_{xy}=\sum_{z\in S}P_{xz}\,P_{zy}$, que es precisamente la
identidad verificada en este numeral mediante los dos métodos de
